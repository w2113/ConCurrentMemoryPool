...
void BenchmarkMalloc(size_t ntimes, size_t nworks, size_t rounds)
{
	...
	std::cout << nworks << "个线程并发执行" << rounds << "轮次,每轮malloc" << ntimes << "次: 花费: "
		<< malloc_costtime << "ms\n";

	/*printf("%u个线程并发执行%u轮次，每轮次free %u次: 花费：%u ms\n",
		nworks, rounds, ntimes, free_costtime);*/
	std::cout << nworks << "个线程并发执行" << rounds << "轮次,每轮free" << ntimes << "次: 花费: "
		<< free_costtime << "ms\n";
	/*printf("%u个线程并发malloc&free %u次，总计花费：%u ms\n",
		nworks, nworks*rounds*ntimes, malloc_costtime + free_costtime);*/
	std::cout << nworks << "个线程并发malloc&free " << nworks * rounds * ntimes << "次，总花费：" <<
...
void BenchmarkConcurrentMalloc(size_t ntimes, size_t nworks, size_t rounds)
{
	...
	std::cout << nworks << "个线程并发concurrent alloc&dealloc "
		<< nworks * rounds * ntimes << "次，总计花费："
		<< malloc_costtime + free_costtime << " ms" << std::endl;
}
